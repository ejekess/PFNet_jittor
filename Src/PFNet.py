...
import Src.backbone.resnet as resnet
...
class CA_Block(nn.Module):
    ...
    def execute(self, x):
        ...
        energy = jt.bmm(proj_query, proj_key)  # [ B, C, C]
        attention = self.softmax(energy)
        proj_value = x.view(m_batchsize, C, -1)  # [B, C, N]

        out = jt.bmm(attention, proj_value)
...
class SA_Block(nn.Module):
    ...
    def execute(self, x):
        ...
        energy = jt.bmm(proj_query, proj_key)
...
class Context_Exploration_Block(nn.Module):
    ...
    def execute(self, x):
        ...
        ce = self.fusion(jt.contrib.concat((p1_dc, p2_dc, p3_dc, p4_dc), dim=1))
...
class Positioning(nn.Module):
    def __init__(self, channel):
        ...
        self.map = nn.Conv(self.channel, 1, 7, 1, 3)
...
class Focus(nn.Module):
    def __init__(self, channel1, channel2):
        ...
        self.output_map = nn.Conv(self.channel1, 1, 7, 1, 3)
        ...
        self.relu2 = nn.ReLU()
    def execute(self, x, y, in_map):
        #x: current-level features, C = channel_1
        #y: higher-level features , C = channel_2 = channel_1*2
        #in_map: higher-level prediction, C=1
...
class PFNet(nn.Module):
    ...
    def execute(self, x):
        ...
        # rescale
        predict4 = nn.interpolate(predict4, size=x.size()[2:], mode='bilinear', align_corners=True)
        predict3 = nn.interpolate(predict3, size=x.size()[2:], mode='bilinear', align_corners=True)
        predict2 = nn.interpolate(predict2, size=x.size()[2:], mode='bilinear', align_corners=True)
        predict1 = nn.interpolate(predict1, size=x.size()[2:], mode='bilinear', align_corners=True)

        if self.training:
            return predict4, predict3, predict2, predict1

        return jt.sigmoid(predict4), jt.sigmoid(predict3), jt.sigmoid(predict2), jt.sigmoid(predict1)
